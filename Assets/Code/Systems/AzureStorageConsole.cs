...
namespace AzureStorageConsole
{
            ...
            try
            {
                ...
                using (HttpWebResponse response = RESTExec("HEAD", Container))
                {
                    if (response.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        Debug.Log("Meta Data for Container: " + Container);
        ...
        public void ListContainers()
        {
            try
            {
                ...
                using (HttpWebResponse response = RESTExec("GET", "?comp=list"))
                {
                    ...
                    if (nodes.Count > 0)
                    {
                        Debug.Log("Containers in Account: " + _Account);
                    ...
                    {
                        Debug.Log("No Containers Defined\n");
                    }
                }
            }
            catch (WebException ex)
    ...
    internal class TableHelper
    {
        ...
        public void ListTables()
        {
            ...
            try
            {
                using (HttpWebResponse res = RESTExec("GET", "Tables"))
                {
                    ...
                    {
                        Debug.Log("Tables in Account: " + _Account);
        ...
        public XmlNodeList GetTable(string Table)
        {
        ...
        public string[] GetPartition(string Table, string partitionKey, string rowKey)
        {
            List<string> returnlist = new List<string>();
            try
            {
                // execute the query
                using (HttpWebResponse res = RESTExec("GET", string.Format("{0}(PartitionKey='{1}',RowKey='{2}')", Table, partitionKey, rowKey)))
                {
                    // load the xml
                    XmlDocument doc = new XmlDocument();
                    doc.Load(res.GetResponseStream());
        ...
        public void InsertRow(string TableName, string PartitionKey, string RowKey, object obj)
        {
            ...
            {
                var errorRespons = (HttpWebResponse)e.Response;
                StreamReader reader = new StreamReader(errorRespons.GetResponseStream());
    ...
    internal class RESTHelper
    {
        ...
        {
            // default account
            if (string.IsNullOrEmpty(Account) || Account.Equals("Account1", StringComparison.OrdinalIgnoreCase)) // REQUIRED-FIELD : replace Account1 with storage account
                return "Account1";
        ...
        {
            if (Account == "Account1") // REQUIRED-FIELD : replace Account1 with storage account and Secret1 with your storage account secret
                return "Secret1";
